    ...
    for i, iframe in enumerate(rss_iframes):
        ...
        if not img_path.exists():
            ...
            if "draw.io" in parsed.netloc or "diagrams.net" in parsed.netloc:
                export_url = iframe_url
                if "viewer.diagrams.net" in iframe_url:
                    export_url = iframe_url.replace("/viewer", "/export")
                export_url += "?format=png&embed=1"
                try:
                    resp = session.get(export_url, timeout=20)
